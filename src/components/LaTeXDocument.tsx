...
function convertLaTeXToHTML(texContent: string): string {
  ...
  let titleHtml = ''
  if (titleMatch) {
    titleHtml += `<h1 class="text-5xl font-bold mb-4 text-white">${escapeHtml(titleMatch[1])}</h1>\n`
  }
  if (authorMatch) {
  ...
  html = html.replace(/\\[a-zA-Z]+(?:\{[^}]*\})?/g, '') // Remove \command{...} and \command
  html = html.replace(/\\[^a-zA-Z]/g, '') // Remove \& \$ \_ etc
  html = html.replace(/\{([^}]*)\}/g, '$1') // Remove bare braces
  html = html.trim()
  ...
  const paragraphs = html
    ...
    .map((para) => {
      para = para.trim()
      if (!para) return ''
      if (/^</.test(para)) return para // Already HTML
